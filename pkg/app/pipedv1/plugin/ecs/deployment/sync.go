...
func (p *ECSPlugin) executeECSSyncStage(
	...
	// TODO: Handle multiple deploy targets later, focus on the main logic first
	// This version of plugin only supports one deploy target, safely get the first one
	client, err := provider.DefaultRegistry().Client(dts[0].Name, dts[0].Config)
	if err != nil {
		lp.Errorf("Failed to get ECS client for deploy target %s: %v", dts[0].Name, err)
		return sdk.StageStatusFailure
	}
	...
	lp.Infof("Service Definition: %+v", serviceDef)
